...
const fb = require('./workers/node_modules/fibonacci');
...
router.get('/fibonacci', (req, res) => {
    const number = fb.iterate(4);
    res.send(number);
});

router.get('/fibonacciThreaded', async (req, res) => {
    runFibonacci({ iterations: 4 }).then(result => log.info(result));
    res.send('processing');
});

router.get('/fibonacciThreadPool', async (req, res) => {
    runFibonacciPool({ iterations: 4 }).then(result => log.info(result));
    res.send('processing');
});
...
router.get('/fibonacciThreadedShared', async (req, res) => {
    ...
    for(let i = 0; i < 4; i++) {
        runFibonacciShared({ iterations: 4, position: i, arr: sharedUint8Array }).then(result => console.log(result));
    }
    res.send('processing');
});
